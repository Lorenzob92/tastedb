...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // Load personal changes from this browser after hydration.
  useEffect(() => {
    const storedOverrides = loadJSON<Record<string, Partial<MediaEntry>>>(OVERRIDES_KEY, {});
    const storedAdditions = loadJSON<MediaEntry[]>(ADDITIONS_KEY, []);
    const frame = window.requestAnimationFrame(() => {
      setOverrides(storedOverrides);
      setAdditions(storedAdditions);
      setReady(true);
    });
  ...
  const media = useMemo(() => buildMedia(overrides, additions), [overrides, additions]);
  ...
  const removeEntry = useCallback(
    (id: string) => {
      setAdditions((prev) => prev.filter((entry) => entry.id !== id));
      setOverrides((prev) => {
        const next = { ...prev };
        delete next[id];
        return next;
